#!/bin/bash
set -euo pipefail
log() { echo "[$(date -Iseconds)] $*" | tee -a logs/sync.log; }
...
# 如果按日文件不存在，兜底用最新一份
if [ ! -f "$csv_path" ]; then
  csv_path=$(ls -1t exports/new_games_*.csv | head -n 1 || true)
fi

if [ -z "${csv_path:-}" ] || [ ! -f "$csv_path" ]; then
  log "No CSV found to import, skipping."
  exit 0
fi

log "Importing CSV: $csv_path"
